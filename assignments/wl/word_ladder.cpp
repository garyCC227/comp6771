...
//curr word must have n char that are same as d_word, otherwise we dont need to search its next level
// n = d_word.size() - 1;
 ...
 */
std::unordered_set<std::string> Dictionary(std::set<std::string> lexicon, std::string word){
...
std::vector<Path> BFS2(const std::string& s_word, const std::string& d_word, std::unordered_set<std::string> lexicon){
    std::queue<Path> path_q;
    std::vector<Path> results_q;
    PossWordMap possible_word_map;

    //init value to search
    int start_diff = NumberOfDiffChar(s_word, d_word);//number of diff char between s_word and d_word
    ...
    while(!path_q.empty()){
        ...
        //if the last node of this branch, the curr_word does not have d_word.size()-1 chars that are same as d_word
        //we can prune, since change one letter will not find the solution
        if(solution_found){
 ...
 */
std::priority_queue<WordType, std::vector<WordType>,CustomCompare> PossibleWord(WordType& word, std::string d_word , std::unordered_set<std::string> lexicon, PossWordMap& possible_word_map){
    ...
    if(possible_word_map.find(curr_word) != possible_word_map.end()){
        ...
        for (auto letter = lexicon.begin(); letter != lexicon.end(); ++letter) {
            ...
            if (cout == 1) {
                ...
                same_char = 0;
                for (size_t i = 0; i < curr_word.size();
                     ++i) { //optimisation method 1, by priority queue
...
void MarkVisited(std::unordered_set<std::string>& lvl_visited, std::unordered_set<std::string>& lexicon){
